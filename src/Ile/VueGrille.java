...
class VueGrille extends JPanel implements Observer {
    ...
    public VueGrille(Modele modele) {
         ...
         * taille d'affichage.
         */
        Dimension dim = new Dimension(TAILLE * dx, TAILLE * dy);
        this.setPreferredSize(dim);
        this.joueur = new Cellule(modele, modele.j.px,modele.j.py);
    }

    /**
     * L'interface [Ile.Observer] demande de fournir une méthode [update], qui
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < Modele.dx; i++) {
            g.setColor(Color.WHITE);
            g.fillRect(i * TAILLE, (Modele.dy - 1) * TAILLE, TAILLE, TAILLE);
        }

        for (int j = 0; j < Modele.dy; j++) {
            g.setColor(Color.WHITE);
        ...
        for (int j = 0; j < Modele.dy; j++) {
            g.setColor(Color.WHITE);
            g.fillRect((Modele.dx - 1) * TAILLE, j * TAILLE, TAILLE, TAILLE);
        }
    ...
    public void paintJoueur(Graphics g) {
        ...
        g.fillOval(this.modele.j.px * TAILLE + (int) 0.25 * TAILLE,
                this.modele.j.py * TAILLE + (int) 0.25 * TAILLE,
                (int) (0.5 * TAILLE), (int) (0.5 * TAILLE));
    }

    /**
     * Fonction auxiliaire de dessin d'une cellule.
     * Ici, la classe [Ile.Cellule] ne peut être désignée que par l'intermédiaire
     * de la classe [CModele] à laquelle elle est interne, d'où le type
     * [CModele.Ile.Cellule].
     * Ceci serait impossible si [Ile.Cellule] était déclarée privée dans [CModele].
     */
